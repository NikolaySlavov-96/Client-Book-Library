...
import {
    ...
    IGetProductRatingResponse,
    IRateProductResponse,
    IGetStatusCountsResponse,
...
export const searchProductByEmailOnUser = async ({ searchContent, page, limit }: ISearchProductByEmailRequest): Promise<ISearchProductByEmailResponse> => api.get(`${SEARCH}/email?email=${searchContent}&limit=${limit}&page=${page}`);
...
export const removeStatusFromProduct = async (productId: string | number) => api.remove(`${PREFIX}/status/${productId}`);

// Rating Services
export const getProductRating = async (id: string | number): Promise<IGetProductRatingResponse> => api.get(`${PREFIX}/${id}/rating`);

export const rateProduct = async (id: string | number, rating: number): Promise<IRateProductResponse> => api.post(`${PREFIX}/${id}/rating`, { inputData: { rating } });